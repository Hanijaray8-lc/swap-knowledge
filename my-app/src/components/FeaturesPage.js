...
export default function FeaturesPage() {
  ...
  const checkForNewMeetings = async (studentName) => {
    try {
      const response = await fetch(`https://swap-knowledge.onrender.com/api/meetings/student/${encodeURIComponent(studentName)}`);
  ...
  const handleSearch = async () => {
    ...
    setIsSearching(true);
    try {
      const response = await fetch(`https://swap-knowledge.onrender.com/api/admin/search-staff?name=${encodeURIComponent(searchQuery)}`);
