          ...
          data.map(async (staffMember) => {
            try {
              ...
              const response = await fetch(
                `https://swap-knowledge.onrender.com/api/follow/status/${staffMember._id}?studentId=${currentStudent._id}`
              );
  ...
  const handleFollowClick = async (staffId, event) => {
    ...
    try {
      const endpoint = isCurrentlyFollowing
        ? `https://swap-knowledge.onrender.com/api/follow/unfollow/${staffId}`
        : `https://swap-knowledge.onrender.com/api/follow/follow/${staffId}`;
