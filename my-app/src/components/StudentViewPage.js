...
export default function StudentViewPage() {
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`https://swap-knowledge.onrender.com/api/student/get-user-by-id/${id}`);
    ...
    const fetchStudentPosts = async () => {
      if (tabValue === 2 && student) {
        try {
          ...
          const response = await axios.get(`https://swap-knowledge.onrender.com/api/posts?author=${student.name}&userType=Student`);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`https://swap-knowledge.onrender.com/api/posts/${postToDelete._id}`);
  ...
  const handleSaveProfile = async () => {
    try {
      setSaving(true);
      const response = await axios.put(`https://swap-knowledge.onrender.com/api/student/update-profile/${id}`, editForm);
                        ...
                        <Card sx={{ 
                          ...
                          {post.imageUrl && (
                            <CardMedia
                              component="img"
                              height="200"
                              image={`https://swap-knowledge.onrender.com${post.imageUrl}`}
                              alt="Post image"
