...
export default function SocialDashboard() {
  ...
  const fetchPosts = async (pageNum) => {
    setLoading(true);
    try {
      const res = await fetch(`https://swap-knowledge.onrender.com/api/posts?page=${pageNum}&limit=15`);
  ...
  const handleShare = async (index) => {
    ...
    if (navigator.share) {
      try {
        ...
        try {
          await axios.post(`https://swap-knowledge.onrender.com/api/posts/${post._id}/share`);
          setPosts(prev =>
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        try {
          const studentRes = await axios.get("https://swap-knowledge.onrender.com/api/swap/student", {
            timeout: 10000 
          });
        ...
        try {
          const staffRes = await axios.get("https://swap-knowledge.onrender.com/api/admin/staff", {
            timeout: 10000 
          });
      ...
      if (matchedStaff && matchedStaff._id) {
        ...
        try {
          const response = await axios.get(
            `https://swap-knowledge.onrender.com/api/admin/get-staff-by-name/${encodeURIComponent(post.name)}`,
            { timeout: 8000 }
          );
                  ...
                  {post.imageUrl && (
                    <Box sx={{ mb: 2 }}>
                      <img
                        ...
                        onClick={() => setSelectedImage(`https://swap-knowledge.onrender.com${post.imageUrl}`)}
                      />
                    </Box>
                  )}
